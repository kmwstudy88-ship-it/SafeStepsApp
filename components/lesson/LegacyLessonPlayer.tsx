...
import { saveReflection } from "../../lib/platform/data";
import {
...
export default function LegacyLessonPlayer({
  lesson,
  lessonId,
  enrolmentId,
}: Props) {
  ...
  useEffect(() => {
    ...
    markLessonInProgress(enrolmentId, lessonId, "reflection").catch(
      () => undefined,
    );
  }, [enrolmentId, lessonId]);
  ...
  async function goNext() {
    try {
      ...
      if (step === "reflection") {
        if (!reflection.trim()) {
          throw new Error("Add a short start reflection first.");
        }
        await saveReflection({
          enrolmentId,
          lessonId,
          phase: "start",
          prompt: "What are you noticing before this lesson?",
          answer: reflection,
        });
      }

      if (step === "checkpoint") {
        const correct = isCheckpointCorrect(
          lesson.checkpoint,
          checkpointAnswer,
        );
        if (correct === false) {
      ...
      if (step === "end_reflection") {
        if (!endReflection.trim()) {
          throw new Error("Add an end reflection before completing.");
        }
        await saveReflection({
          enrolmentId,
          lessonId,
          phase: "end",
          prompt: "What will you practice after this lesson?",
          answer: endReflection,
        });
        if (enrolmentId) {
          await completeLessonAndUnlockNext(enrolmentId, lessonId);
        }
        Alert.alert(
          "Lesson complete",
          "Your reflection and progress have been saved.",
        );
        router.back();
        return;
      }
      ...
      if (next) {
        setStep(next);
        if (enrolmentId) {
          await markLessonInProgress(enrolmentId, lessonId, next);
        }
      }
    } catch (error) {
      Alert.alert(
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.topRow}>
        ...
        <Text style={styles.pagePill}>
          Page {stepIndex + 1} of {steps.length}
        </Text>
      </View>
      ...
      {step === "reflection" && (
        <Card title="Start reflection" accent={palette.accent}>
          <Text style={styles.body}>
            Before learning, write what is happening for you right now and
            what you want to get from this lesson.
          </Text>
          <Input
            value={reflection}
      ...
      {step === "checkpoint" && (
        <Card title="Checkpoint" accent={palette.accent}>
          ...
          <Input
            ...
            placeholder="Your checkpoint answer..."
          />
          {!!feedback && (
        ...
        <Card title="Scenario" accent={palette.accent}>
          <Text style={styles.body}>
            {lesson.scenario?.situation ||
              "Scenario has not been added yet."}
          </Text>
          <Text style={styles.body}>
          ...
          <Input
            ...
            onChangeText={setScenarioAnswer}
            placeholder="Write what you would do..."
            multiline
          />
        </Card>
      )}
      ...
      {step === "practice" && (
        <Card
          title={lesson.practice?.title || "Practice task"}
          accent={palette.accent}
        >
          <Text style={styles.body}>
            {lesson.practice?.instruction ||
              "Write one practical step you will try today."}
          </Text>
          <Input
            value={practiceAnswer}
            onChangeText={setPracticeAnswer}
            placeholder="Write your practice plan..."
            multiline
          />
        </Card>
      )}
      ...
      {step === "end_reflection" && (
        <Card title="End reflection" accent={palette.accent}>
          <Text style={styles.body}>
            Finish by writing what changed in your thinking and what you will
            practice next.
          </Text>
          <Input
            value={endReflection}
      ...
      <View style={styles.actions}>
        <Pressable
          style={[
            ...
            !getPreviousLessonStep(step) && styles.disabled,
          ]}
          onPress={goBack}
          disabled={!getPreviousLessonStep(step)}
        >
