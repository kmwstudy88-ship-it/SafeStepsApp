...
-- Harden execute permissions without broad wildcard revokes.
-- We use an explicit allowlist of confirmed admin-only helper functions.
do $$
declare
  admin_only_function text;
begin
  foreach admin_only_function in array array[
    ...
    if to_regprocedure(admin_only_function) is not null then
      execute format('revoke execute on function %s from public, anon, authenticated', admin_only_function);
      execute format('grant execute on function %s to service_role', admin_only_function);
    end if;
  end loop;
end;
$$;

-- These RPCs intentionally remain callable for unauthenticated verification and
-- report delivery. Keep narrow grants and lock execution context.
...
revoke execute on function public.consume_case_report_delivery(text, text)
  from public;
revoke execute on function public.verify_real_video_report_delivery(text, text)
  from public;
revoke execute on function public.verify_video_report_public(text)
  from public;
