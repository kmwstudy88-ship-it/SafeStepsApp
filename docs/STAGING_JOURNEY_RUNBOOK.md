...
Production remains blocked until this matrix passes with synthetic staging data and the Security and Performance Advisors are reviewed. Set `SAFESTEPS_SECURITY_ADVISOR_REVIEWED_AT`, `SAFESTEPS_PERFORMANCE_ADVISOR_REVIEWED_AT`, and `SAFESTEPS_STAGING_JOURNEY_VERIFIED_AT` only after that evidence is complete, then run `npm run release:gate`.
