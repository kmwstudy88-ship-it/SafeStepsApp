...
select has_function('public', 'grant_parent_video_report_consent', array['uuid', 'text', 'text', 'timestamptz'], 'grant_parent_video_report_consent exists');
...
select ok(
  has_function_privilege('authenticated', 'public.grant_parent_video_report_consent(uuid,text,text,timestamptz)', 'EXECUTE'),
  'authenticated can execute grant_parent_video_report_consent'
);
select ok(
  not has_function_privilege('anon', 'public.grant_parent_video_report_consent(uuid,text,text,timestamptz)', 'EXECUTE'),
  'anon cannot execute grant_parent_video_report_consent'
);
select ok(
  has_function_privilege('authenticated', 'public.withdraw_parent_video_report_consent(uuid,text)', 'EXECUTE'),
  'authenticated can execute withdraw_parent_video_report_consent'
);
select ok(
  not has_function_privilege('anon', 'public.withdraw_parent_video_report_consent(uuid,text)', 'EXECUTE'),
