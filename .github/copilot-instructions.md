...
- If a resumed session, saved plan, prior instruction, or carried-forward agent/task model parameter references a retired or unavailable explicit model choice, discard that explicit override before any new turn or tool call and continue with `auto`; if an explicit model is still necessary afterward, choose only a currently supported runtime model.
