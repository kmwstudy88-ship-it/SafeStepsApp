-- Qualify pgcrypto calls because pgcrypto is installed in the extensions schema.
...
begin
 ...
 loop
  random_value:=('x'||encode(extensions.gen_random_bytes(4),'hex'))::bit(32)::bigint;
 ...
 ) values(p_case_id,rel.id,rec.id,encode(extensions.digest(raw_token,'sha256'),'hex'),
  encode(extensions.digest(raw_code,'sha256'),'hex'),now()+make_interval(mins=>p_expires_minutes),
...
begin
 ...
 if c.revoked_at is not null then outcome:='revoked';
 ...
 elsif c.code_hash<>encode(extensions.digest(coalesce(p_code,''),'sha256'),'hex') then
  ...
  if rel.id is null then outcome:='unavailable';
  elsif exists(select 1 from public.report_withdrawals w where w.report_id=rel.report_id and w.report_version=rel.report_version) then
   outcome:='withdrawn';
  else
