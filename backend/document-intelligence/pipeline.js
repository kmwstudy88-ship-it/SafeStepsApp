...
async function createDocumentRecord({ userId, caseId, fileName, mimeType, buffer, extractedText, sourceType }) {
  ...
  const { error: storageError } = await admin.storage.from('document-intelligence').upload(storagePath, buffer, {
  ...
  const { data: document, error } = await admin.from('documents').insert({
    ...
    metadata: { extraction: extractedText ? 'inline' : 'unavailable' },
  }).select('*').single();
  if (error) {
...
async function processAnalysisJob(job) {
  ...
  if (!document.extracted_text) throw new Error('This file type needs text extraction before AI analysis. Supply extractedText when uploading binary documents.');

  await Promise.all([
    admin.from('documents').update({ processing_status: 'processing', updated_at: new Date().toISOString() }).eq('id', document.id),
    admin.from('document_analyses').update({ status: 'processing', started_at: new Date().toISOString() }).eq('id', analysisId),
  ]);

  const ai = await analyzeDocument(document);
  ...
  const { error } = await admin.from('document_analyses').update({
    ...
    fairness: result.fairness || {}, limitations: result.limitations || [], raw_output: result, usage: ai.usage || {}, completed_at: now,
  }).eq('id', analysisId);
  if (error) throw error;
  await admin.from('documents').update({ processing_status: 'completed', updated_at: now }).eq('id', document.id);
}
